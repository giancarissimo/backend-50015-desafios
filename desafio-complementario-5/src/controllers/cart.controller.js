...
class CartController {
    ...
    async addProductToCart(req, res) {
        ...
        const quantity = req.body.quantity || 1
        const quantitySpeed = req.body.quantitySpeed
        const userId = req.user._id // Nos traemos el ID del usuario logueado en el momento y lo almacenamos en la variable UserId
        try {
            // Se verifica si el carrito existe en el array de carritos
            const verifyCartId = await cartServices.getCartById(cartId)
            if (!verifyCartId) {
            ...
            const user = !req.user.googleId ? await UserModel.findById(userId) : await GoogleUserModel.findById(userId)
            if (user.role === 'premium') {
                // Se verifica si el producto pertenece al usuario
                const product = verifyProductId

                if (product.owner === user.email) {
                    // Si el producto pertenece al usuario premium, se retorna un error
